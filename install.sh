...
#                 and non-interactive command-line operations.
...
EOF
    restart_xray
    display_result "$p_port" "$p_uuid" "$p_sni" "$ip" "$public_key" "$shortid" "$p_no_qrcode"
}
        ...
        --install)
            ...
            while [[ $# -gt 0 ]]; do
              case "$1" in
                ...
                --sni) p_sni="$2"; shift 2;;
                --no-qrcode) p_no_qrcode="true"; shift;;
                *) error "安装时使用了未知选项: $1";;
              esac
            done
            ;;
